<template>
    ...
    <div class="row g-2" style="margin-top: 15px;">
        <div class="col-md">
            <div class="form-floating">
                ...
                <label for="floatingSelectGrid_Race">选择比赛场次</label>
            </div>
        </div>
    </div>

    <div v-if="leaderboard && leaderboard.length > 0">
        <p class="form-text" id="raceMainPeriodText" style="margin-top: 0;">当前比赛时段：{{ currentOcmRacePeriod }}</p>
        <p class="form-text" id="raceStartAtText" style="margin-top: -20px;">比赛开始：{{ raceStartAt }}</p>
        <p class="form-text" id="raceEndAtText" style="margin-top: -20px;">比赛结束：{{ raceEndAt }}</p>
        <p class="form-text" id="raceFetchTimeText" style="margin-top: -20px;">数据获取时间：{{ this.leaderboard[0].fetchTime
            }}</p>
        <p class="form-text" id="fetchInfoText" style="margin-top: -20px;">※ 每5分钟获取一次数据 | 每页显示{{ this.itemsPerPage
            }}名次数据</p>
    </div>
    ...
    <div class="row">
        <div class="col">
            <button type="button" id="reloadButton" class="btn btn-primary" @click="getOcmRaceRanking()" hidden>
        ...
        <tbody>
            <tr class="table-row" v-for="(entry, index) in getPaginatedData()" :key="index">
                ...
                <td :class="getRowClass(index)" style="width: 20%;">
                    <div style="font-weight: bolder;">{{ entry.raceResult }}</div>
                </td>
                ...
                <td :class="getRowClass(index)" style="width: 20%; font-size: xx-small;">{{ entry.car.title }}</td>
    ...
    <!-- 手机端布局 -->
    <table class="table table-striped mobile_table" id="mobile_table" style="width: 100%; table-layout: fixed;">
        <tbody>
            <tr class="rankingBody mobile_table_body" v-for="(entry, index) in getPaginatedData()" :key="index">
                ...
                <td class="title form-text" style="font-size: 10px;">{{ entry.car.title }}</td>
                <td class="result form-text">
                    <div style="font-weight: bolder;">{{ entry.raceResult }}</div>
                </td>
...
export default {
    data() {
        return {
            ...
            currentOcmRacePeriod: '未开始',
            raceStartAt: '1970/1/1 00:00:00',
            raceEndAt: '1970/12/31 00:00:00',
        }
    },
    methods: {
        getOcmRaceRanking() {
            this.leaderboard = [];
            // 禁用下拉框
            document.getElementById('floatingSelectGrid_Race').setAttribute('disabled', 'disabled');
            // 显示加载中文本
            ...
            axios.post('/api/ocmRaceResult', {
                ...
                .then(response => {
                    if (response.data.fetchStatus === true) {
                        ...
                        if (response.data.isMainDraw === true) {
                            if (response.data.isInTallyPeriod === true) {
                                this.currentOcmRacePeriod = '计算中......';
                            } else {
                                this.currentOcmRacePeriod = '正赛';
                            }
                        } else {
                            this.currentOcmRacePeriod = '预赛';
                        }
                        if (response.data.isRaceOver === true) {
                            this.currentOcmRacePeriod = '已结束';
                        }
                        this.raceStartAt = response.data.startAt;
                        this.raceEndAt = response.data.endAt;
                    } else {
                        this.leaderboard = [];
                ...
                .finally(() => {
                    // 恢复下拉框
                    document.getElementById('floatingSelectGrid_Race').removeAttribute('disabled');
